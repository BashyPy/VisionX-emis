...
from functions import init_db, display_first_aid_guide_auto_scroll_with_manual, connect_to_google_sheet, log_scan_activity, create_qr_code, get_patient_by_id, insert_or_update_patient, validate_phone_and_emergency_contact, validate_phone, validate_nin, validate_emergency_contact
import warnings
...
scan_worksheet = connect_to_google_sheet(SHEET_ID, 'Scan Activities')
...
if "patient_id" in query_params:
    ...
    if patient:
        ...
        # 2/3 width for patient info, 1/3 for image
        col1, col2 = st.columns([2, 1])

        # Column 1: Display patient information
        with col1:
            # Patient name
            st.subheader(f"Medical Information for {patient[2]}")
            ...
            st.markdown('\n'.join(
                f"- {entry.strip()}" for entry in medical_history_entries if entry.strip()))
        ...
        with col2:
            ...
            st.image(image, caption="First Aid in Action",
    ...
    with st.form("patient_form"):
        ...
        nin_input = st.text_input("National Identification Number (NIN)",
        ...
        emergency_contact_input = st.text_input("Emergency Contact Number",
    ...
    if submitted:
        if consent:
            if not validate_nin(nin_input):
                st.error("NIN must be exactly 11 digits.")
            else:
                phone_number = validate_phone(phone_input)
                emergency_contact_number = validate_emergency_contact(emergency_contact_input)

                if not phone_number:
                    st.error("Invalid phone number format.")
                elif not emergency_contact_number:
                    st.error("Invalid emergency contact number format.")
                elif not validate_phone_and_emergency_contact(phone_number, emergency_contact_number):
                    # Error message is handled within the validator
                    pass
                else:
                    patient_id = f"PAT{nin_input[-4:]}{age}{phone_input[-4:]}"

                    # Insert or update patient and get the QR link (URL)
                    qr_link = insert_or_update_patient(
                        name, age, nin_input, phone_number, emergency_contact_number, genotype, blood_type,
